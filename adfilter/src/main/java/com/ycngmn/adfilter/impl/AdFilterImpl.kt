...
import com.ycngmn.adblockclient.ResourceType
import com.ycngmn.adfilter.AdFilter
import com.ycngmn.adfilter.DownloadState
import com.ycngmn.adfilter.Filter
import com.ycngmn.adfilter.FilterResult
import com.ycngmn.adfilter.impl.Constants.FILE_STORE_DIR
import com.ycngmn.adfilter.impl.Constants.KEY_ALREADY_UP_TO_DATE
import com.ycngmn.adfilter.impl.Constants.KEY_FILTERS_COUNT
import com.ycngmn.adfilter.impl.Constants.KEY_FILTER_NAME
import com.ycngmn.adfilter.impl.Constants.KEY_RAW_CHECKSUM
import com.ycngmn.adfilter.impl.Constants.TAG_INSTALLATION
import com.ycngmn.adfilter.script.ElementHiding
import com.ycngmn.adfilter.script.ScriptInjection
import com.ycngmn.adfilter.script.Scriptlet
import com.ycngmn.adfilter.util.None
...
internal class AdFilterImpl(appContext: Context) : AdFilter {
    ...
    private val elementHiding: ElementHiding = ElementHiding(appContext,detector)
    private val scriptlet: Scriptlet = Scriptlet(detector, context = appContext)
    ...
    init {
        viewModel.isEnabled.observeForever { enable ->
            if (enable) {
                ...
                filterDataLoader.load(FilterDataLoader.Companion.ID_CUSTOM)
            } else {
    ...
    override fun performScript(webView: WebView?, url: String?) {
        if (viewModel.isEnabled.value == true) {
            elementHiding.perform(webView)
            scriptlet.perform(webView)
        }
    }
}